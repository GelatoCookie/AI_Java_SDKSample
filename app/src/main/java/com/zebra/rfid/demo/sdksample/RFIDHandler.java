...
class RFIDHandler implements Readers.RFIDReaderEventHandler {
    ...
    private void connectReader() {
        executor.execute(() -> {
            ...
            synchronized (RFIDHandler.this) {
                if (!isReaderConnected()) {
                    GetAvailableReader();
                    String result = (reader != null) ? connect() : (context != null ? context.getString(R.string.failed_to_find_reader) : "Failed to find reader");
                    if (context != null) {
                        context.updateReaderStatus(result, isReaderConnected());
                    }
                } else {
    ...
    private synchronized String connect() {
        if (reader != null) {
            try {
                ...
                return context != null ? context.getString(R.string.connection_failed, e.getMessage()) : "Connection failed";
            } catch (OperationFailureException e) {
                Log.e(TAG, "Connection failed", e);
                return context != null ? context.getString(R.string.connection_failed, e.getStatusDescription()) : "Connection failed";
            }
        }
    ...
    public void scanCode() {
        ...
        executor.execute(() -> executeCommand(DCSSDKDefs.DCSSDK_COMMAND_OPCODE.DCSSDK_DEVICE_PULL_TRIGGER, in_xml, scannerID));
    }
